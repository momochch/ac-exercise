...
const dataList = document.querySelector('#list-bar')
const dataTh = document.querySelector('#list-th')
...
//按list bar 未完成
dataList.addEventListener('click', function onPanelClicked(event) {
  if (event.target.matches('#list-bar')) {
    showMovieModal(Number(event.target.dataset.id))
  }
  else if (event.target.matches('.btn-add-favorite')) {
    addToFavorite(Number(event.target.dataset.id))
